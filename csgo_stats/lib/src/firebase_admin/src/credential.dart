...
import 'package:csgo_stats/src/firebase_admin/src/utils/env.dart';
...
class Credentials {
  ...
  static Future<Credential> login({
    ...
    var client = Client(
      issuer,
      clientId ?? const String.fromEnvironment('FIREBASE_CLIENT_ID'),
      clientSecret: clientSecret ??
          const String.fromEnvironment('FIREBASE_CLIENT_SECRET'),
    );
    ...
    f.writeAsStringSync(const JsonEncoder.withIndent(' ').convert(v));
  ...
  static String? get firebaseAdminCredentialPath {
    ...
    return path.join(
      config,
      'firebase_admin/application_default_credentials.json',
    );
  }
